...
from core.models.db_models import Evaluation, Issue, Job
...
class EvaluationOrchestrator:
    ...
    def _infer_banking_domain(self, document_type: str, document_text: str) -> str | None:
        """Best-effort fallback when LLM domain classification is unavailable."""
        text = f"{document_type or ''} {document_text or ''}".lower()
        if any(k in text for k in ("kyc", "aml", "onboarding", "beneficial owner", "pep", "sanctions")):
            return "Customer Onboarding (KYC/AML)"
        if any(k in text for k in ("loan", "credit", "facility", "collateral", "covenant", "borrower")):
            return "Loan & Credit Documentation"
        if any(k in text for k in ("treasury", "liquidity", "lcr", "nsfr", "hql", "hqda")):
            return "Treasury & Liquidity Reports"
        if any(k in text for k in ("regulatory", "pillar", "disclosure", "crr", "crd", "finrep", "corep")):
            return "Regulatory & Compliance Filings"
        if any(k in text for k in ("m&a", "merger", "acquisition", "fairness opinion", "investment banking")):
            return "Investment Banking & M&A"
        if any(k in text for k in ("fraud", "investigation", "sar", "fincen", "whistleblower")):
            return "Fraud & Investigation Records"
        return None
    ...
    async def evaluate_document(
        ...
        if not banking_domain:
            inferred_domain = self._infer_banking_domain(document_type, combined_text)
            if inferred_domain:
                banking_domain = inferred_domain
                logger.info("Banking domain inferred from content: %s", banking_domain)
