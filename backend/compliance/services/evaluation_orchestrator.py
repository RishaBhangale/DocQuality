...
from core.services.rule_engine import execute_rule
...
class EvaluationOrchestrator:
    ...
    async def evaluate_document(
        ...
        logger.info("Extracted %d characters", len(document_text))

        # ── Step 2: Classify → Silver layer ──────────────────────────────
        logger.info("Step 2: Classifying document semantic type...")
        t1 = time.time()
        from compliance.config import SEMANTIC_TYPES
        semantic_type = self.llm_service.classify_semantic_type(document_text, SEMANTIC_TYPES)
