"""
Unified Pydantic schemas for request/response validation.

Merges Banking and Compliance schemas into a single set:
- IssueSchema: union of both field sets
- MetricResult: Compliance's version with id, category, linked_standards (Banking adopts)
- EvaluationResponse: Compliance structure + Banking domain fields as optional extras
- JobResponse, JobStatus: from Banking (Compliance adopts)
- BankingMetric: domain-specific banking metric
- LLM schemas: from both workspaces
"""
...
class IssueSchema(BaseModel):
    """Unified schema for a single detected issue."""
...
class MetricResult(BaseModel):
    """
    Unified schema for a single quality metric evaluation result.

    Uses Compliance's richer structure (with id, category, linked_standards).
    Banking adopts this format.
    """
...
# ─── LLM Extraction Response (Compliance-style) ────────────────────────────
...
class LLMExtractionResponse(BaseModel):
    ...
    # Optional banking domain evaluation
    domain_evaluation: Optional[dict] = Field(
        default=None,
        description="Banking-domain metric scores and reasoning keyed by short metric code.",
    )


# ─── Strict LLM Quality Schemas (Banking-style) ───────────────────────────
...
class LLMStrictQualityResponse(BaseModel):
    ...
    banking_domain: Optional[str] = Field(
        default=None,
        description="Detected banking domain (optional)",
    )
...
class LLMConsolidationResponse(BaseModel):
    """Final consolidation output for recommendations + issues."""
...
class BankingMetric(BaseModel):
    ...
    regulatory_pass_threshold: Optional[int] = Field(
        default=None,
        description="Score needed to pass regulatory standard",
    )
    regulatory_reference: str = Field(
        "", description="Regulation this threshold derives from"
    )
    ...
    deterministic_score: int = Field(0, description="Raw deterministic score before blending")
...
class EvaluationResponse(BaseModel):
    """
    Unified full evaluation response returned to the client.

    Compliance structure as the base, Banking fields as optional extras.
    """
    ...
    metrics: list[MetricResult]  # all metrics combined
    ...
    created_at: datetime
    # Banking domain intelligence — populated only for banking workspace
    banking_domain: Optional[str] = Field(
        default=None,
        description="Detected banking domain",
    )
    ...
    banking_overall_score: Optional[float] = Field(
        default=None,
        description="S_Bank composite score (0–100)",
    )
