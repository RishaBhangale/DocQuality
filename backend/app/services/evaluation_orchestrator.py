...
from app.models.db_models import Evaluation, Issue, Job
...
from app.services.banking_rule_engine import (
    BankingRuleEngine,
)
...
def evaluate_document_job(job_id: str, file_path: str, filename: str) -> None:
    ...
    """
    from app.database import SessionLocal  # local import to avoid startup order issues
...
class EvaluationOrchestrator:
    ...
    async def evaluate_document(
        ...
        filename: str,
        db: Session,
        eval_mode: str = "compliance",
        job_id: str | None = None,
    ) -> EvaluationResponse:
        """
        Execute the full evaluation pipeline handling both generic info and banking specific workflows.

        Args:
            file_path: Path to the saved document on disk.
            filename: Original upload filename.
            db: Database session.
            eval_mode: 'compliance' or 'banking', decides if banking agent flows run.
        ...
        completeness_score, completeness_issues = self.rule_engine.calculate_completeness(fields, combined_text)
        det_metric_scores["completeness"] = completeness_score
        all_issues.extend(completeness_issues)

        validity_score, validity_issues = self.rule_engine.calculate_validity(fields, combined_text)
        det_metric_scores["validity"] = validity_score
        all_issues.extend(validity_issues)

        consistency_score, consistency_issues = self.rule_engine.calculate_consistency(fields, combined_text)
        ...
        timeliness_score, timeliness_issues = self.rule_engine.calculate_timeliness(fields, combined_text)
        det_metric_scores["timeliness"] = timeliness_score
        all_issues.extend(timeliness_issues)

        uniqueness_score, uniqueness_issues = self.rule_engine.calculate_uniqueness(fields, combined_text)
        ...
        # If the user selected generic compliance mode, ignore any detected banking domain.
        banking_domain: str | None = pre_classified_domain if eval_mode == "banking" else None
