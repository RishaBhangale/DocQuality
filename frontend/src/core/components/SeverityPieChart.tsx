...
interface SeverityPieChartProps {
  issues: Issue[];
  /** Omit outer card + title when nested in a dashboard panel. */
  embedded?: boolean;
}
...
export function SeverityPieChart({ issues, embedded }: SeverityPieChartProps) {
  const counts: Record<string, number> = { critical: 0, warning: 0, good: 0 };
  (issues || []).forEach((issue) => {
    const sev = (issue.severity || '').toLowerCase() as keyof typeof counts;
  ...
  const chartBody = (
      <div className={`w-full ${embedded ? 'h-[260px]' : 'h-[300px] sm:h-[350px]'}`} style={{ overflow: 'visible' }}>
