...
function normalizeIssue(raw: Record<string, unknown>): Issue {
  ...
  return {
    fieldName: String(raw.field_name ?? raw.fieldName ?? ''),
    issueType: String(raw.issue_type ?? raw.issueType ?? ''),
    description: String(raw.description ?? ''),
    severity,
    regulationReference: (raw.regulation_reference ?? raw.regulationReference) as string | undefined,
    metricDimension: (raw.metric_dimension ?? raw.metricDimension) as string | undefined,
    metricName: (raw.metric_name ?? raw.metricName) as string | undefined,
  };
}
...
export function IssuesTable({ issues: rawIssues, showRegulation = true }: IssuesTableProps) {
  const issues = useMemo(
    () => (rawIssues || []).map((i) => normalizeIssue(i as Record<string, unknown>)),
    [rawIssues],
  );
  ...
  const dimensions = useMemo(() => {
    const all = issues
      .map((i) => i.metricName || i.metricDimension)
  ...
  const displayed = useMemo(() => {
    ...
    if (filterDimension !== 'all') {
      filtered = filtered.filter(
        (i) => (i.metricName || i.metricDimension) === filterDimension,
      );
    }
  ...
  if (issues.length === 0) {
    return (
      <div className="bg-white rounded-xl p-8 shadow-sm border border-gray-100/90">
  ...
  const colCount = showRegulation ? 6 : 5;

  return (
    <div className="bg-white rounded-xl p-6 sm:p-8 shadow-sm border border-gray-100/90">
      {/* Header */}
        ...
        <div className="inline-block min-w-full align-middle px-6 sm:px-0">
          <table className="w-full min-w-[980px] table-fixed md:min-w-0">
            <colgroup>
              ...
              <col className="w-[14%]" />
              <col className="w-[32%]" />
              {showRegulation && <col className="w-[12%]" />}
              <col className="w-[12%]" />
            </colgroup>
            <thead>
              <tr className="border-b border-gray-200">
                <th className="text-left py-3 px-3 sm:px-4 text-xs sm:text-sm font-semibold text-gray-700">Field</th>
                <th className="text-left py-3 px-3 sm:px-4 text-xs sm:text-sm font-semibold text-gray-700 whitespace-nowrap">Metric</th>
              ...
              {displayed.length === 0 ? (
                ...
                displayed.map((issue, index) => (
                  ...
                  >
                    ...
                    <td className="py-4 px-3 sm:px-4 text-xs sm:text-sm align-top whitespace-normal break-words">
                      {issue.metricName || issue.metricDimension ? (
                        <span className="text-blue-600 font-medium">{issue.metricName || issue.metricDimension}</span>
                      ) : (
