...
import { Loader2, FileText, Building2, Download, History, ChevronDown, ChevronUp, ClipboardList, ShieldAlert, CalendarDays, FolderOpen, ShieldCheck, ArrowLeftRight } from 'lucide-react';
...
export default function App() {
  // ── Workspace Selection ───────────────────────────────────────────────────────
  const [evalMode, setEvalMode] = useState<'compliance' | 'banking' | null>(null);

  const handleWorkspaceSelect = (mode: 'compliance' | 'banking') => {
    setEvalMode(mode);
  };

  const handleSwitchWorkspace = () => {
    // Clear all state when switching workspaces
    setEvalMode(null);
  ...
  const handleEvaluate = async () => {
    ...
    try {
      ...
      formData.append('eval_mode', evalMode ?? 'compliance');
      ...
      // Check if the backend returned a direct evaluation result or a job reference
      if (data.evaluation_id && data.overall_score !== undefined) {
        // Direct evaluation response — set result immediately
        setEvaluationResult(data as EvaluationResult);
        setIsEvaluating(false);
        setProgressMessage('');
      } else if (data.job_id) {
        // Job-based async flow — start polling
  ...
  // Show splash screen if no workspace selected yet
  if (!evalMode) {
    return <SplashPage onSelect={handleWorkspaceSelect} />;
  }

  const isBankingMode = evalMode === 'banking';
  const modeLabel = isBankingMode ? 'Banking Intelligence Quality' : 'AI Governance & Compliance';
  const modeSubtitle = isBankingMode ? 'Domain-specific financial document analysis' : 'ISO standards compliance evaluation';
      ...
      <header className="relative bg-white border-b border-slate-200">
        <div className="mx-auto flex max-w-[1320px] items-center justify-between px-6 py-6 sm:px-10 xl:px-12">
          <div className="flex items-center gap-3">
            <div className={`w-10 h-10 rounded-lg flex items-center justify-center ${isBankingMode ? 'bg-emerald-700' : 'bg-[#1E3A8A]'}`}>
              {isBankingMode ? <Building2 className="w-6 h-6 text-white" /> : <ShieldCheck className="w-6 h-6 text-white" />}
            </div>
            <div>
              <span className="text-xl font-semibold text-gray-900">{modeLabel}</span>
              <p className="text-xs uppercase tracking-[0.18em] text-slate-400">{modeSubtitle}</p>
            </div>
          </div>
          <div className="flex items-center gap-3">
            {evaluationResult && (
              ...
              <div className="animate-in fade-in">
                ...
                <div className="mb-6">
                  <BankingRadarChart
                    ...
                    bankingDomain={evaluationResult.banking_domain ?? undefined}
                  />
                </div>
